...
#' 
#' @reAlign A logical indicating whether to perform veritical re-alignment of 
#' fixations (default is TRUE)
#' 
...
MultiLine<- function(data_list= NULL, ResX= 1920, ResY=1080, maxtrial= 120, 
                     tBlink= 50, reAlign=TRUE, RSpar= c(1/4, 8, 2/3), textStim= NULL,
                     ppl= NULL, xOffset=NULL, plot=FALSE){
  ...
  for (i in 1:length(data)){ # for each subject..
    ...
    for(j in 1:nrow(trial_db)){ # for each item
      ...
      if(text[1]!=0 | hasArg(textStim)){ # if trial contained text
        ...
        try(raw_fix_temp<- parse_fix(file, map=0, coords=0, trial_db[j,], i, ResX, ResY, tBlink, hasText=FALSE, SL= F))
        if(is.null(raw_fix_temp)){
          next;
        }
        raw_fix_temp$dataFile= filename
